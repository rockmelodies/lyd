...
EOQ;
...
}else if(isset($_POST['payMethod']) && ($_POST['payMethod'] == 'PAYPAL')){
	$amount = $_SESSION['amount'] = number_format(50,2,'.','');
	$payment_method = $_SESSION['payment_method'] = 'credit_card';
	$currency = $_SESSION['currency'] = 'EUR';
	$descriptions = array( $amount.' '.$currency.' Adminitration Fee');
	...
	$paypal = new paypal();
	$paypal->paymentAmount = urlencode($amount);
	$paypal->returnURL = urlencode('http://lydormarkets.com/account/paypal_success.php');
	$paypal->cancelURL = urlencode('http://lydormarkets.com/account/payment.php');
	$paypal->desc = $descriptions;
	$paypal->currency = $currency;
	$paypal->items = $items;
	$SetExpressCheckout = $paypal->SetExpressCheckout();
	if(!empty($SetExpressCheckout['ERROR'])){
		$_SESSION['paypal_error_message'] = $SetExpressCheckout['ERROR']['L_LONGMESSAGE0'];
